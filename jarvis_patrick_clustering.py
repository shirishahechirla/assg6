...
def jarvis_patrick_clustering():
    ...
    plt.title(f'Clustering for Dataset {best_dataset_index} \n (Highest ARI) - k value :{best_k} and s_min: {best_s_min}')
    ...
    plt.title(f'Clustering for Dataset {best_dataset_index_sse} \n (Lowest SSE) - k value :{best_k} and s_min: {best_s_min}')
    plt.suptitle('Jarvis Patrick Clustering')
    plt.xlabel('Feature 1')
    plt.ylabel('Feature 2')
    plt.colorbar(label='Cluster')
